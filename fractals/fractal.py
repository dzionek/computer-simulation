...
class Fractal(metaclass=ABCMeta):
    """
    Abstract class initializes fractals, checks if values from a complex plane make convergent sequences,
    ...
    Attributes:
        ...
        real_domain: Tuple[float, float] - the domain of real numbers which we explore
        imaginary_domain: Tuple[float, float] - the domain of imaginary numbers which we explore (without imaginary units!)
    """
    def __init__(self, iteration_limit: int, accuracy: int,
    ...
    def last_convergent_array(self) -> List[List[int]]:
        """
    ...
    def plot(self) -> None:
        ...
        plt.imshow(last_convergent_array.T, cmap='RdGy', interpolation='bilinear',
                   extent=self.real_domain + self.imaginary_domain)
        plt.show()